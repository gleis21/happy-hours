...
const repo = require("../repositories/repository")(
  process.env.TEST_SERVICE_ACCOUNT_KEY,
...
describe("Time records repository", function() {
  describe("adding, getting and deleting records", function() {
    it("adds and deletes a record", function() {
      ...
      return repo
        .addNewTimeRecord(row)
        .then(() =>
          expect(
            repo.getTimeRecordsByEmail("test@test.com")
          ).to.eventually.eql([row]))
        .then(() => repo.deleteRowById(id))
        .then(() =>
          expect(
            repo.getTimeRecordsByEmail("test@test.com")
          ).to.have.eventually.length(0));
    });
  });
});
